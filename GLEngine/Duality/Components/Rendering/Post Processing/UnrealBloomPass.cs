...
namespace Duality.Postprocessing
{
	public class UnrealBloomPass : Pass, IDisposable
	{
		...
		public UnrealBloomPass(Vector2? resolution=null, float? strength=null, float? radius=null, float? threshold=null) : base()
		{
			...
			Resolution = resolution != null && resolution.HasValue ? resolution.Value : new Vector2(256, 256);
			this.ClearColor = new Color(0, 0, 0);

			// render targets
			Hashtable pars = new Hashtable { { "minFilter", THREE.Constants.LinearFilter }, { "magFilter", THREE.Constants.LinearFilter }, { "format", THREE.Constants.RGBAFormat } };
		...
		{
			this.oldClearColor = DualityApp.GraphicsBackend.GetClearColor();
			...
			DualityApp.GraphicsBackend.SetClearColor(this.ClearColor, 0);
			...
			DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetBright);
			...
			for (var i = 0; i < this.nMips; i++)
			{
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.RenderTargetsHorizontal[i]);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.RenderTargetsVertical[i]);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
			...
			DualityApp.GraphicsBackend.SetRenderTarget(this.RenderTargetsHorizontal[0]);
			DualityApp.GraphicsBackend.Clear();
			this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
