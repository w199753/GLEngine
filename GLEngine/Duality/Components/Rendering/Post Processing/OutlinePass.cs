...
namespace Duality.Postprocessing
{
	public class OutlinePass : Pass,IDisposable
	{
		...
		THREE.Math.Matrix4 textureMatrix;
		...
		public OutlinePass(Vector2 resolution, List<Object3D> selectedObjects = null) : base()
		{
			...
			this.maskBufferMaterial.Side = THREE.Constants.DoubleSide;
			...
			this.depthMaterial.Side = THREE.Constants.DoubleSide;
			...
			this.prepareMaskMaterial.Side = THREE.Constants.DoubleSide;
			this.prepareMaskMaterial.FragmentShader = ReplaceDepthToViewZ(this.prepareMaskMaterial.FragmentShader, this.camera );
			...
			((this.separableBlurMaterial1.Uniforms["texSize"] as GLUniform)["value"] as THREE.Math.Vector2).Set((float)resx,(float)resy);
			(this.separableBlurMaterial1.Uniforms["kernelRadius"] as GLUniform)["value"] = 1;
			this.separableBlurMaterial2 = this.GetSeperableBlurMaterial(MAX_EDGE_GLOW );
			((this.separableBlurMaterial2.Uniforms["texSize"] as GLUniform)["value"] as THREE.Math.Vector2).Set((float)System.Math.Round(resx / 2),(float)System.Math.Round(resy / 2 ));
			...
			this.textureMatrix = new THREE.Math.Matrix4();
		}
		...
		public override void Render(GLRenderTarget writeBuffer, GLRenderTarget readBuffer, bool? maskActive = null)
        {
			if (this.selectedObjects.Count > 0)
			{
				...
				this.oldClearAlpha = DualityApp.GraphicsBackend.GetClearAlpha();
				var oldAutoClear = DualityApp.GraphicsBackend.AutoClear;

				DualityApp.GraphicsBackend.AutoClear = false;

				if (maskActive!=null && maskActive.Value) DualityApp.GraphicsBackend.state.buffers.stencil.SetTest(false);

				DualityApp.GraphicsBackend.SetClearColor(Color.Hex(0xffffff), 1);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetDepthBuffer);
				...
				((this.prepareMaskMaterial.Uniforms["cameraNearFar"] as GLUniform)["value"] as THREE.Math.Vector2).Set(this.camera.Near, this.camera.Far);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetMaskBuffer);
				DualityApp.GraphicsBackend.Clear();
				DualityApp.GraphicsBackend.Render(this.scene, this.camera);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetMaskDownSampleBuffer);
				DualityApp.GraphicsBackend.Clear();
				...
				((this.edgeDetectionMaterial.Uniforms["texSize"] as GLUniform)["value"] as THREE.Math.Vector2).Set(this.renderTargetMaskDownSampleBuffer.Width, this.renderTargetMaskDownSampleBuffer.Height);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetEdgeBuffer1);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetBlurBuffer1);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetBlurBuffer2);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
				...
				DualityApp.GraphicsBackend.SetRenderTarget(this.renderTargetEdgeBuffer2);
				DualityApp.GraphicsBackend.Clear();
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);
				...
				if (maskActive!=null && maskActive.Value) DualityApp.GraphicsBackend.state.buffers.stencil.SetTest(true);

				DualityApp.GraphicsBackend.SetRenderTarget(readBuffer);
				this.fullScreenQuad.Render(DualityApp.GraphicsBackend);

				DualityApp.GraphicsBackend.SetClearColor(this.oldClearColor, this.oldClearAlpha);
        ...
        public override void SetSize(float width, float height)
        {
			...
			var texSize = (this.separableBlurMaterial1.Uniforms["texSize"] as GLUniform)["value"] as THREE.Math.Vector2;
			...
			texSize = (this.separableBlurMaterial2.Uniforms["texSize"] as GLUniform)["value"] as THREE.Math.Vector2;
			texSize.X = resx; texSize.Y = resy;
		}
